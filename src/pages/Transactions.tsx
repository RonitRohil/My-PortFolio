        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-nowrap gap-2 overflow-x-auto pb-1">
            ...
            {accounts.map((account) => (
              <Pill key={account.id} active={activeAccounts.has(account.id)} onClick={() => toggleAccount(account.id)}>
                {account.bankName}
              </Pill>
            ))}
          </div>
                ...
                <div className="-m-6">
                  ...
                  <div className="divide-y divide-slate-800/60">
                    {group.items.map((item) => {
                      ...
                      return (
                        <div key={`${tx.type}-${tx.id}`} className="flex flex-col justify-between gap-4 px-6 py-4 md:flex-row md:items-start">
                          ...
                          <div className="flex items-center gap-3">
                            <div className={`text-right font-bold ${tx.type === "income" ? "text-emerald-400" : tx.type === "expense" ? "text-rose-400" : "text-slate-200"}`}>
